// ============================================================
// LOOT HANDLER — Coleta de recursos
//
...
import { grantXP, RESOURCE_ACTION }         from '../services/SkillEngine';
...
export function registerLootHandler(socket: Socket): void {
  socket.on('player:collect', (intent: CollectIntent) => {
    ...
    if (!node || node.respawnAt !== null || node.quantity <= 0) {
      socket.emit('collect:fail', { reason: 'Recurso indisponível', resourceId: intent.resourceId });
      return;
    }

    // ── 2. Player está em alcance? ────────────────────────────
    if (!inRange(player.position, node.position, COLLECT_RANGE)) {
      socket.emit('collect:fail', { reason: 'Muito longe do recurso', resourceId: intent.resourceId });
      return;
    }
    ...
    if (maxByWeight <= 0) {
      socket.emit('collect:fail', { reason: 'Mochila cheia', resourceId: intent.resourceId });
      return;
    }
    ...
    // Adiciona ao inventário de sessão (agrupa por tipo)
    ...
    socket.emit('collect:ok', {
      ...
      xpResults,   // para o HUD de progressão mostrar ganhos de XP
    });
