...
import { db }                  from '../db';
import { globalEconomy }       from '../db/schema';
...
// Lido do banco na inicialização, atualizado a cada transação,
// recalculado periodicamente. Evita reads por transação.
...
let marketCache = new Map<string, MarketEntry>(); // itemType → entry
...
// ── Recalcula preços periodicamente ──────────────────────────
...
export function recalculatePrices(): void {
  ...
  for (const entry of marketCache.values()) {
    ...
    // Nova multiplicação sobre o preço base
    let   mult = 1 + pressure;

    // Drift de volta ao base quando há pouca atividade
    const currentMult = entry.currentPrice / entry.basePrice;
    mult = currentMult + (mult - currentMult) * (1 - DRIFT_RATE);
...
export async function buyItem(
  ...
  const unitPrice = entry.currentPrice;
  const totalCost = Math.round(unitPrice * quantity * 100) / 100;

  // Carrega dados do player (moeda + stash)
  ...
  if (dbPlayer.currency < totalCost) {
    return {
      ok:     false,
      reason: `Moeda insuficiente (você tem ${dbPlayer.currency.toFixed(2)}, precisa de ${totalCost.toFixed(2)})`,
    };
  }

  // Verifica peso do stash
  ...
  // Persiste stash, moeda e estatísticas de mercado atomicamente
  db.transaction(tx => {
    // Débita moeda
    tx.update(require('../db/schema').players)
      .set({ currency: newBalance })
      .where(eq(require('../db/schema').players.id, playerId))
      .run();

    // Atualiza stash
    PlayerRepository.saveStash(playerId, newStash, stashVersion);

    // Decrementa estoque + registra compra
    tx.update(globalEconomy)
      .set({
        stockQuantity:    entry.stockQuantity - quantity,
        boughtSinceReset: entry.boughtSinceReset + quantity,
        updatedAt:        new Date(),
      })
      .where(eq(globalEconomy.id, entry.id))
      .run();
...
export async function sellItem(
  ...
  // Preço de venda = 60% do preço atual (spread do Trader)
  ...
  // Verifica se tem os itens no stash
  const stash        = (dbPlayer.stash?.items as StashData) ?? [];
  const stashVersion = dbPlayer.stash?.version ?? 1;
  ...
  db.transaction(tx => {
    ...
    tx.update(globalEconomy)
      .set({
        stockQuantity:  entry.stockQuantity + quantity,
        soldSinceReset: entry.soldSinceReset + quantity,
        updatedAt:      new Date(),
      })
      .where(eq(globalEconomy.id, entry.id))
      .run();
...
function addToStash(
  stash: StashData, itemType: string, qty: number, weight: number,
): StashData {
...
function removeFromStash(
  stash: StashData, itemType: string, qty: number,
): StashData {
  const unitWeight = ITEM_WEIGHT_TABLE[itemType as keyof typeof ITEM_WEIGHT_TABLE] ?? 1;
  return stash
