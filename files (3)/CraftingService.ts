...
export async function executeCraft(
  ...
  if (!recipe) {
    return { ok: false, reason: 'Receita desconhecida' };
  }
  ...
  if (!dbPlayer?.stash) {
    return { ok: false, reason: 'Stash não encontrado' };
  }
  ...
  for (const ingredient of recipe.inputs) {
    ...
    if (!stack || stack.quantity < ingredient.quantity) {
      ...
      return {
        ok:     false,
        reason: `Precisa de ${ingredient.quantity}× ${meta?.label ?? ingredient.itemType}`,
      };
    }
  }

  // ── 5. Consome ingredientes (em RAM) ──────────────────────
...
function addToStash(
  ...
  if (existing) {
    ...
    copy.push({ itemType, quantity, weight });
  }
