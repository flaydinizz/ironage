...
export const traderRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // ── GET /trader/market ────────────────────────────────────
  // Retorna todos os itens com preço atual, estoque e variação
  fastify.get('/market', async (_request, reply) => {
    ...
    const market = snapshot.map(entry => ({
      ...
      sellPrice:    Math.round(entry.currentPrice * 0.6 * 100) / 100, // spread Trader
    }));
