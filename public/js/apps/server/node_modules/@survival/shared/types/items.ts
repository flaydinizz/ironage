...
export type CraftableType =
  ...
  | 'chest'           // wood → baú simples early game para organização
  | 'storage_chest';  // wood_plank + iron_ingot → baú avançado, permite transferir itens para o stash

export type ItemType = ResourceType | WeaponType | ArmorType | CraftableType;

// ── Stack de item (inventário / stash) ───────────────────────
...
export interface ItemMeta {
  ...
  // Efeito ao usar o item diretamente (bandage, water_flask, food_ration)
  onUse?: {
    restoreHp?:     number;
...
export const ITEM_META: Record<ItemType, ItemMeta> = {
  ...
  chest:           { label: 'Baú',                  category: 'craftable'  },
  storage_chest:   { label: 'Baú Stash',            category: 'craftable'  },
};

export const DEFAULT_STASH_WEIGHT_LIMIT = 500; // kg
