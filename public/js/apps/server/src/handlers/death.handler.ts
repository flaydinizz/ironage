...
//
// Chamado por:
//   - tick.loop   (morte por desidratação ou efeito de status)
...
// Timer de respawn variável entre 5-10s (Etapa 11)
const RESPAWN_DELAY_MIN_MS = 5_000;
const RESPAWN_DELAY_MAX_MS = 10_000;

function getRandomRespawnDelay(): number {
  return Math.floor(
    Math.random() * (RESPAWN_DELAY_MAX_MS - RESPAWN_DELAY_MIN_MS) + RESPAWN_DELAY_MIN_MS
  );
}

// Guarda socketIds em processo de respawn para evitar duplo-trigger
...
export function handlePlayerDeath(
  ...
  const respawnDelay = getRandomRespawnDelay();

  // Notifica o player (tela de morte no cliente)
  io.to(player.socketId).emit('player:died', {
    message,
    lostItems,
    respawnDelayMs: respawnDelay,
  });
  ...
  console.log(
    `[Death] ${player.username} morreu | ${lostItems.length} slots perdidos | killer: ${killerName ?? 'ambiente'}`
  );

  // ── Respawn após delay ────────────────────────────────────
  setTimeout(() => {
    // Player pode ter desconectado durante o delay
