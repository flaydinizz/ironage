...
//
// Design: mantém um Map<playerId, SkillsData> em memória.
// Evita reads desnecessários no banco durante a sessão.
// O dirty set marca quais players precisam de flush.
// ============================================================
...
const ACTION_XP_TABLE: Record<GameAction, XPGrant[]> = {
  ...
  survive_tick:        [{ skill: 'hunting',   xp: 1  }],
  use_stealth:         [{ skill: 'stealth',   xp: 5  }],
};

// Mapeamento de resourceType → GameAction (para o loot handler)
...
// Retorna skills atuais da RAM (sem I/O)
...
export function grantXP(
  ...
  if (!skills) { skills = { ...DEFAULT_SKILLS }; sessionSkills.set(playerId, skills); }
...
// Remove da RAM ao desconectar (sempre faz flush antes)
