...
import type { StatusEffect }                      from '../game/fauna';
...
export const VITALS = {
  ...
  // HP não regenera sozinho (precisa de item de cura)
...
export function drainVitals(
  player:    SessionPlayer,
  dtSeconds: number,
): { died: boolean; thirstDrained: number; hpDamage: number } {
  ...
  return { died: player.hp <= 0, thirstDrained, hpDamage };
}
...
export function tickStatusEffects(
  ...
  player.activeEffects = player.activeEffects.filter(effect => {
    if (now >= effect.expiresAt) { expired.push(effect.stackId); return false; }
    totalDamage += effect.damagePerSec * dtSeconds;
    return true;
  });

  if (totalDamage > 0) player.hp = Math.max(0, player.hp - totalDamage);
